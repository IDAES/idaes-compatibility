...
def test_example():
    ...
    m.fs.properties_2 = iapws95.Iapws95ParameterBlock(
    ...
    assert value(m.fs.feed_1.mole_frac_comp[0, "benzene"]) == pytest.approx(
        0.6, rel=1e-6
    )
    assert value(m.fs.feed_1.mole_frac_comp[0, "toluene"]) == pytest.approx(
        0.4, rel=1e-6
    )

    ## Case 2:
    m.fs.feed_2 = FeedFlash(
        property_package=m.fs.properties_2, flash_type=FlashType.isenthalpic
    )
