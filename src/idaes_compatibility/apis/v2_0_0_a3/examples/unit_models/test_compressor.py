...
from pyomo.environ import ConcreteModel, SolverFactory, value, units, TerminationCondition, SolverStatus
...
def test_example():
    ...
    assert m.fs.compr_case_1.work_isentropic[0].value == pytest.approx(135439976.18, rel=1e-5)
    assert m.fs.compr_case_1.work_mechanical[0].value == pytest.approx(159341148.45, rel=1e-5)
    ...
    assert m.fs.compr_case_2.work_isentropic[0].value == pytest.approx(135439779.20953986, rel=1e-5)
    assert m.fs.compr_case_2.work_mechanical[0].value == pytest.approx(159340916.71710572, rel=1e-5)
